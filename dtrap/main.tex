...
\documentclass[acmlarge, manuscript, screen, review, anonymous]{acmart}
...
For example, the protocol in Figure~\ref{fig:example-vac-secure} implements the following function:
...
The ideal functionality implemented by the protocol in in Figure~\ref{fig:example-insecure}
...
For example, \inlinecode{C}'s ideal world view of an execution of the protocol in Figure~\ref{fig:example-vac-protocol}
...
Continuing the example of Figure~\ref{fig:example-vac-protocol}, \inlinecode{C}'s real world view would additionally contain
...
\begin{figure}[tbhp]
  ...
  \caption{An example protocol written in \langname.
...
\begin{table}
  ...
  \begin{tabular}{| c c c c |}
    \hline
      \textbf{Ideal Functionality} & \textbf{Implementation} & \textbf{MPC Secure?} & \\
      \hdashline
      \multicolumn{2}{| c}{\textit{Ideal World Views}} & \multicolumn{2}{c |}{\textit{Real World Views}}  \\
    \hline
      $\mathsf{PMF}\big[F_1(\mathtt{sec})\big] := \mathsf{u}(\mathtt{0}, \mathtt{1})$ &
        Figure~\ref{fig:example-insecure} &
        No &
        \\\hdashline
        \multicolumn{2}{| c}{\begin{tabular}{l l}
            $\mathsf{Ideal}_\mathtt{C}\bigpar{F_1,\vertvec{\mathtt{sec}:\mathtt{0}}}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{x}:\mathtt{0} }, \vertvec{ \mathtt{x}:\mathtt{1} }}$ \\
            $\mathsf{Ideal}_\mathtt{C}\bigpar{F_1, \vertvec{ \mathtt{sec}:\mathtt{1} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{x}:\mathtt{0} }, \vertvec{ \mathtt{x}:\mathtt{1} }}$
        \end{tabular}} &
        \multicolumn{2}{c |}{\begin{tabular}{l l}
            $\mathsf{Real}_\mathtt{C}\bigpar{F_1, \vertvec{ \mathtt{sec}:\mathtt{0} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{flp}:\mathtt{0} \\ \mathtt{x}:\mathtt{0} },
                               \vertvec{ \mathtt{flp}:\mathtt{1} \\ \mathtt{x}:\mathtt{1} }}$ \\
            $\mathsf{Real}_\mathtt{C}\bigpar{F_1, \vertvec{ \mathtt{sec}:\mathtt{1} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{\mathtt{flp}:\mathtt{0} \\ \mathtt{x}:\mathtt{1}},
                               \vertvec{\mathtt{flp}:\mathtt{1} \\ \mathtt{x}:\mathtt{0}}}$
        \end{tabular}}
        \\
      \hline
      $\mathsf{PMF}\big[F_2(\mathtt{sec})\big] := \mathsf{u}(\mathtt{0}, \mathtt{1})$ &
        Figure~\ref{fig:example-vac-protocol} &
        Yes &
        \\\hdashline
        \multicolumn{2}{| c}{\begin{tabular}{l l}
            $\mathsf{Ideal}_\mathtt{C}\bigpar{F_2,\vertvec{\mathtt{sec}:\mathtt{0}}}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{x}:\mathtt{0} }, \vertvec{ \mathtt{x}:\mathtt{1} }}$ \\
            $\mathsf{Ideal}_\mathtt{C}\bigpar{F_2, \vertvec{ \mathtt{sec}:\mathtt{1} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{x}:\mathtt{0} }, \vertvec{ \mathtt{x}:\mathtt{1} }}$
        \end{tabular}} &
        \multicolumn{2}{c |}{\begin{tabular}{l l}
            $\mathsf{Real}_\mathtt{C}\bigpar{F_2, \vertvec{ \mathtt{sec}:\mathtt{0} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{\_un}:\mathtt{0} \\ \mathtt{x}:\mathtt{0} },
                               \vertvec{ \mathtt{\_un}:\mathtt{0} \\ \mathtt{x}:\mathtt{1} },
                               \vertvec{ \mathtt{\_un}:\mathtt{1} \\ \mathtt{x}:\mathtt{0} },
                               \vertvec{ \mathtt{\_un}:\mathtt{1} \\ \mathtt{x}:\mathtt{1} }}$ \\
            $\mathsf{Real}_\mathtt{C}\bigpar{F_2, \vertvec{ \mathtt{sec}:\mathtt{1} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{\_un}:\mathtt{0} \\ \mathtt{x}:\mathtt{0} },
                               \vertvec{ \mathtt{\_un}:\mathtt{0} \\ \mathtt{x}:\mathtt{1} },
                               \vertvec{ \mathtt{\_un}:\mathtt{1} \\ \mathtt{x}:\mathtt{0} },
                               \vertvec{ \mathtt{\_un}:\mathtt{1} \\ \mathtt{x}:\mathtt{1} }}$
        ...
        \\
      \hline
      $F_3(\mathtt{sec}, \mathtt{pub}) := \mathtt{sec} \neq \mathtt{pub}$ &
        Figure~\ref{fig:example-vac-secure} &
        Yes &
        \\\hdashline
        \multicolumn{2}{| c}{\begin{tabular}{l l}
            $\mathsf{Ideal}_\mathtt{C}\bigpar{F_3,\vertvec{ \mathtt{sec}:\mathtt{0} \\ \mathtt{pub}:\mathtt{0} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{pub}:\mathtt{0} \\ \mathtt{x}:\mathtt{0} }}$ \\
            $\mathsf{Ideal}_\mathtt{C}\bigpar{F_3,\vertvec{ \mathtt{sec}:\mathtt{0} \\ \mathtt{pub}:\mathtt{1} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{pub}:\mathtt{1} \\ \mathtt{x}:\mathtt{1} }}$ \\
            $\mathsf{Ideal}_\mathtt{C}\bigpar{F_3,\vertvec{ \mathtt{sec}:\mathtt{1} \\ \mathtt{pub}:\mathtt{0} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{pub}:\mathtt{0} \\ \mathtt{x}:\mathtt{1} }}$ \\
            $\mathsf{Ideal}_\mathtt{C}\bigpar{F_3,\vertvec{ \mathtt{sec}:\mathtt{1} \\ \mathtt{pub}:\mathtt{1} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{pub}:\mathtt{1} \\ \mathtt{x}:\mathtt{0} }}$ \\
        \end{tabular}} &
        \multicolumn{2}{c |}{\begin{tabular}{l l}
            $\mathsf{Real}_\mathtt{C}\bigpar{F_3, \vertvec{ \mathtt{sec}:\mathtt{0} \\ \mathtt{pub}:\mathtt{0} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{pub}:\mathtt{0} \\ \mathtt{x}:\mathtt{0} }}$ \\
            $\mathsf{Real}_\mathtt{C}\bigpar{F_3, \vertvec{ \mathtt{sec}:\mathtt{0} \\ \mathtt{pub}:\mathtt{1} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{pub}:\mathtt{1} \\ \mathtt{x}:\mathtt{1} }}$ \\
            $\mathsf{Real}_\mathtt{C}\bigpar{F_3, \vertvec{ \mathtt{sec}:\mathtt{1} \\ \mathtt{pub}:\mathtt{0} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{pub}:\mathtt{0} \\ \mathtt{x}:\mathtt{1} }}$ \\
            $\mathsf{Real}_\mathtt{C}\bigpar{F_3, \vertvec{ \mathtt{sec}:\mathtt{1} \\ \mathtt{pub}:\mathtt{1} }}$ & \negquad\!\!\! $\leftsquigarrow
             \mathsf{u}\bigpar{\vertvec{ \mathtt{pub}:\mathtt{1} \\ \mathtt{x}:\mathtt{0} }}$ \\
        \end{tabular}}
        \\
    \hline
  \end{tabular}
  \bigskip
    \caption{A summary of some introductory examples of protocols.
    The exact structure with which views are represented is purely a matter of notation;
    here we use flat vectors of \inlinecode{variable:value} bindings,
    and we omit duplicates.
    We denote a uniform distribution across a finite list as $\mathsf{u}(\dots)$,
    and $\leftsquigarrow$ denotes sampling.
    \inlinecode{0} and \inlinecode{1} are synonymous with \inlinecode{false} and \inlinecode{true} respectively.
    Since $F_3$ is deterministic, we define it directly as a function instead of a Probability Mass Function ($\mathsf{PMF}$);
    that \inlinecode{C}'s views of its implementation happen to also be deterministic is incidental.}
  \label{tbl:examples}
\end{table}
%}
...
For example consider the protocol in Figure~\ref{fig:example-vac-protocol}.)
...
For example the protocol in Figure~\ref{fig:example-vac-secure} is vacuously secure,
...
the choice an engineering judgment.
...
%
$$
\bigpar{ \vec{y}_H \centernot{⫫} \mathsf{Real}_C\bigpar{Π, \vec{x}} \mid \mathsf{Ideal}_C\bigpar{F,\vec{x}} }
\rightarrow
\nexists \; \mathtt{SIM}
$$
%
...
assuming the program correctly implements its intended functionality
and that \toolname is configured with sufficient power for the target program,
\toolname can identify a program as not MPC secure.
...
most of the protocols \toolname \emph{can} detect insecure will be \emph{unambiguously} insecure.
...
and $\alpha$ is actually an \emph{upper bound} on the likelyhood of falsely flagging a protocol as insecure.
