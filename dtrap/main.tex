...
\begin{figure}[tbhp]
  ...
  %\newcommand{\gsize}{.38\textwidth}
    ...
    Its ideal functionality is \textit{"flip a coin"},
    because the one-bit output is uniformly distributed regardless of anyone's \inlinecode{SECRET}s.}
  \label{fig:example-insecure}
  \Description[.CHO code for a protocol for two parties, yielding random output.]
              {Eight lines of .CHO code. Two parties participate; they both get a random value "x" at the end, but the secret value "sec" is leaked.}
\end{figure}
\begin{figure}[tbhp]
  \begin{mdframed}
  ...
  \caption{An example protocol, Protocol~\ref{fig:example-vac-protocol}, written in \langname.
    This protocol \emph{does} demonstrate MPC security, although its ideal functionality is just \textit{"flip a coin"}.}
  \label{fig:example-vac-protocol}
  \Description[.CHO code for a protocol for two parties, yielding random output.]
              {Eight lines of .CHO code. Two parties participate; they both get a random value "x" at the end, after some extra shenanigans.}
\end{figure}
\begin{figure}[tbhp]
  \begin{mdframed}
  ...
  \Description[.CHO code for a protocol for two parties, yielding the "XOR" of two inputs.]
              {Eight lines of .CHO code. Two parties participate; they're each able to learn the other's input.}
\end{figure}
...
on the kinds of functionality to which the definition could be applied\footnote{
    ...
    MPC is not a solution for situations where that's not the case.
}.
...
Certain edge cases are helpful to consider, as they map out the edges of the problem of testing for MPC security.
...
\begin{figure}[tbhp]
  ...
  \label{fig:views}
  \Description[Three formatted CSV tables, in which each row corresponds to a possible trace through the respective program.]
              {Three formatted CSV tables. Each row represents an execution trace. The honest secrets are in red.
               Data that's in the ideal world views is in green; additional data that's in the real world views is in grey.}
\end{figure}
...
\begin{figure}[tbhp]
  ...
  \Description{Four quadrants, for when DT-SIM correctly or incorrectly says "INSECURE" or "MAYBE SECURE".}
\end{figure}
...
party | var | func  :=  regex("[a-zA-Z_][a-z0-9_]*")
  ...
  \caption{Syntax of the \langname language.
      ...
      are omitted.}
  \label{fig:cho-bnf}
  \Description{A semi-formal BNF syntax for the .CHO language.}
\end{figure}
...
OUTPUT h_out2
  ...
  \Description{A 19-line .CHO program; it doesn't do much but it shows how the language is used.}
\end{figure}
...
\begin{figure*}[tbhp]
  ...
  \label{fig:sankey}
  \Description[A Sankey plot showing how DT-SIM assesed 500 protocols at varying power settings.]
              {A Sankey plot showing how DT-SIM assesed 500 protocols.
              As the test power was increased, more and more of the protocols were flagged insecure,
              until at "trainN=4813" the last protocol was flagged "INSECURE" at "alpha=0.000125".}
\end{figure*}
...
\begin{figure}[tbhp]
  ...
  \Description[A simple line chart showing runtime as a function of test power.]
              {A simple line chart showing runtime as a function of test power.
              The relationship is clearly linear, and variation within any given power is low.}
\end{figure}
...
\label{fig:cube-search}
  \Description[3D heat-maps showing the affects of the three individual test-power settings on performance.]
              {3D heat-maps showing the affects of the three individual test-power settings on performance.
              One heat-map shows how the settings affect DT-SIM's ability to detect insecure programs.
              The other shows how the settings affect the runtime of DT-SIM.
              Both are show twice so the viewer can see the front and back of the cubes.}
\end{figure*}
...
\begin{figure}
  ...
  \label{fig:protocols}
  \Description{Two small tables describing the frameworks and functionalities used in this experiment.}
\end{figure}
...
\begin{figure}
  ...
  \label{fig:errors}
  \Description{A table of the different combinations of framework, functionality, and mutation,
               to easily show how each mutation affects the true security of the protocols.}
\end{figure}
...
\begin{figure}
  ...
  \Description{A table with the same twenty-four cells as the previous table, showing what DT-SIM reported in that case.}
\end{figure}
...
\caption{Experimental results from running \toolname against GMW implementations of Beaver triple generation.
    \textit{(a.)} Keeping \texttt{iters=128}, \texttt{trainN=1024}, and \texttt{testN=512}, we report the average of the returned $p$-values
    ...
    \textit{(b.)} We report the average runtime of \toolname with different power-settings against protocols of different bit-widths.
      ...
      In \textit{(b.)} only the un-mutated version is analyzed, and \texttt{testN=trainN/2}}.
\label{fig:observations-detail}
  \Description{Two line-graphs, showing the p-values and runtimes from DT-SIM during one of the bit-width-series in this experiment.}
\end{figure*}
%sourced from choreography/graphs/Adder Circuit Results.ipynb
...
%\section{Example \langname program}
\label{sec:example-big}

This Appendix contains additional experimental results excluded from the main body of the paper, and a larger \langname example.

Figure~\ref{fig:results_training_time} shows detailed runtime data for the six un-mutated (secure) functionality/framework combinations.
Figures~\ref{fig:extra1}, \ref{fig:extra2}, and~\ref{fig:extra3} present detailed $p$-value results for
$n$-bit addition, $n$-bit less-than comparison, and $n$-bit Beaver triple generation, respectively.
The experiments were run with four combinations of \toolname parameters and at increasing bit-widths;
in all cases the runtime was a simple factor of the total amount of data used.
$p$-values can be broadly characterized as having three behaviors:
tightly distributed near zero (\texttt{INSECURE}), uniformly distributed in the entire range of $0-1$ (\texttt{MAYBE SECURE}),
or tightly distributed near one (also \texttt{MAYBE SECURE});
at higher bit-widths some behavior intermediate between the two \texttt{MAYBE SECURE} behaviors is observed.

In Figure~\ref{fig:example-less-than} we show a \langname program that
uses oblivious-transfer based GMW to evaluate a circuit representing a "less than" function
...
the \langname protocols we used included some "no-op" statements where mutations would later be inserted.
Figure~\ref{fig:example-less-than-csv} shows the actual rows of view-data that are produced and fed into \toolname during a test.
...
\label{fig:results_training_time}
  \Description{Six line-graphs, each with data-series plotted, all showing simple linear relations between total-data and running time.}
\end{figure*}
...
\begin{figure}[tbhp]
  \begin{mdframed}
      \begin{small}
      \lstinputlisting[language=cho]{lt_secure.cho}
      \end{small}
  \end{mdframed}
  %\newcommand{\gsize}{.38\textwidth}
  \caption{An implementation of a 2-bit "less than" function, using the GMW framework for MPC, written in \langname.}
  \label{fig:example-less-than}
  \Description{A larger .CHO program, like the ones used in our experiments.}
\end{figure}
...
\begin{figure}[tbhp]
  \begin{mdframed}
    ...
    numberstyle={\color{gray}\itshape},
    numbersep=15px,
          breaklines=true]{lt_secure.csv}
      \end{small}
  \end{mdframed}
  %\newcommand{\gsize}{.38\textwidth}
    \caption{The first fourty-four lines of view data generated by running the protocol in Figure~\ref{fig:example-less-than} in our Haskell semantics.
    This is a verbatim CSV file generated by the Haskell executable and consumed by \toolname, except truncated to fit on one page.}
  \label{fig:example-less-than-csv}
